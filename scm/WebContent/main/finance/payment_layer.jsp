...
<script type="text/javascript">
	...
	function showDetail(poid, tr) {
		...
		if (nextTr.css("display") == "none") {
			nextTr.find("td").load("main/finance/pomainDetail", {
				"poid" : poid
			}, function() {
...
<table width="100%" border="0" align="center" cellspacing="1" class="c">
	...
	<c:forEach items="${page.dataList }" var="po">
		<tr ondblclick="showDetail(${po.poId},this)" title="双击查看详情">
			...
			<td align="center"><c:if test="${po.status ==1 }">新增</c:if> <c:if
					test="${po.status ==2 }">已收货</c:if> <c:if test="${po.status ==3 }">已付款</c:if>
				<c:if test="${po.status ==4 }">已了结</c:if> <c:if
					test="${po.status ==5 }">已预付</c:if></td>

			<td align="center"><a
				href="javascript:payment(${po.poId},<c:if test="${po.payType == '1' || po.payType=='2' || (po.payType == '3' && po.status ==2)}">1</c:if><c:if test="${po.payType == '3' && po.status ==1}">2</c:if>)">

					<c:if test="${po.payType == '1' || po.payType=='2'}">付款登记</c:if> <c:if
						test="${po.payType == '3' && po.status ==1}">预付款登记</c:if> <c:if
						test="${po.payType == '3' && po.status ==2}">尾款登记</c:if>
			</a></td>
		</tr>
		<tr style="display: none">
			<td colspan="11" name="detail"></td>
		</tr>
	</c:forEach>
</table>
<%--导入分页层 --%>
